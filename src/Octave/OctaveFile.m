...
This Octave file is meant to solve the non - convex Quadratically Constrained
Quadratic Program (QCQP) of which the solution to the task offloading
decision is formulated within the paper.  This is accomplished by using
semidefinite relaxation to convert the QCQP into a Semidefinite
Programming (SDP) problem.

Setting up the Octave enviornment, installing SeDuMi, and installing
necessary packages in order to make this code run on your machine may take a
little but of time.  But if you wish to do so the instructions are below.
...
%%% Helper Functions %%%
% These functions are for output and for implementing the algorithm from the
% paper
...
function printIOData(fileName)
    ...
    global dataOut;
    outFile = fopen(fileName, "w");
    fprintf(outFile, "%s\n", "Data In vector then Data Out vector");
...
% Transmission energy to tranmit task with given size and energy rate
...
function aSubCPrime = aSubCPrime(); % Tested
    global remoteTrans;
    global fSubC;
    aSubCPrime = (1 / 2) * ((1 / remoteTrans) .* (diag(dSubIn() + ...
        dSubOut())) + (1 / fSubC) .* (diag(dSubIn()) * diag(dSubOut())));
    return
endfunction
...
function bNot = bNot(); % Tested
    ...
    global constR;
    global numTasks;
    bNot =[(-eSubL() + eSubT + eSubR() + (constA .* cSubK()))', ...
        zeroPrime(numTasks)', constR]';
    return
endfunction
...
function bSubAPrime = bSubAPrime(); % Tested
    global localTrans;
    global fSubA;
    bSubAPrime = (1 / localTrans) .* dSubIn() + ...
        (1 / localTrans) .* dSubOut() ...
        + (1 / fSubA) .* (diag(dSubIn()) * App());
    return
endfunction
...
function bSubC = bSubC(); % Tested
    global localTrans;
    global numTasks;
    bSubC =[((1 / localTrans) .* dSubIn() + (1 / localTrans) * dSubOut())', ...
    zeroPrime(numTasks)', -1]';
    return
endfunction
...
function gSubL = gSubL() % Tested
        global numTasks;
        gSubL =[zeros(numTasks * 2 + 1, numTasks * 2 + 1), ...
            (1 / 2) .* bSubL(); (1 / 2) .* bSubL()', 0];
        return
endfunction
